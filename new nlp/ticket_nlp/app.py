...
.pill {
    ...
    border-radius: 20px; display: inline-block; letter-spacing: .04em;
}
...
/* ── decision badge ── */
.dec-ok   { display:inline-flex; align-items:center; gap:5px; font-size:11px;
            color:#3fb950; background:#0d2010; border:1px solid #1a4726;
            border-radius:6px; padding:3px 9px; margin-top:2px; }
.dec-warn { display:inline-flex; align-items:center; gap:5px; font-size:11px;
            color:#d29922; background:#201800; border:1px solid #4b3200;
            border-radius:6px; padding:3px 9px; margin-top:2px; }

/* ── solutions list ── */
.sol-list { list-style:none; padding:0; margin:0; }
.sol-list li {
    font-size:12px; color:#c9d1d9; padding:4px 0;
    border-bottom:1px solid #161b22;
...
/* ── empty state ── */
.empty-state {
    text-align:center; color:#30363d; font-size:13px;
    padding:50px 20px;
    border: 1px dashed #161b22; border-radius:10px;
    margin-top: 10px;
}
...
.bubble-ai {
    background:#0d1117;
    border:1px solid #1c2128;
    color:#cdd9e5; font-size:13px; line-height:1.7;
    padding:14px 16px;
...
RULE_KEYWORDS = {
    "Server & Infrastructure Issue": ["server", "database", "outage", "infrastructure", "down", "production"],
    "Security Issue": ["breach", "hack", "ransomware", "malware", "security", "phishing", "unauthorized"],
    "Network & VPN Issue": ["vpn", "network", "internet", "firewall", "wifi", "bandwidth", "latency"],
    "Account & Access Issue": ["login", "password", "account", "access", "reset", "locked", "credentials", "sign in", "2fa", "mfa"],
    "Software & App Issue": ["app", "software", "crash", "bug", "error", "update", "patch", "version"],
    "Hardware Issue": ["hardware", "printer", "laptop", "monitor", "keyboard", "mouse", "device"],
    "Billing & Payment Issue": ["billing", "invoice", "payment", "refund", "charge", "subscription"],
}
...
def extract_entities(text: str) -> dict:
    ...
    module = (
        ...
        "Database"    if any(k in t for k in ["database","sql","mysql","postgres"]) else
        "VPN"         if any(k in t for k in ["vpn","network","wifi","internet"]) else
        "API"         if any(k in t for k in ["api","endpoint"]) else
        "Application" if any(k in t for k in ["application","software","app"]) else
        "Auth"        if any(k in t for k in ["login","password","account","reset","credentials"]) else
        "N/A"
    )
    platform = (
        "AWS"     if "aws"     in t else
        "Azure"   if "azure"   in t else
        "GCP"     if "gcp"     in t else
        "Windows" if "windows" in t else
        "Linux"   if "linux"   in t else
        "macOS"   if "macos"   in t else
        "N/A"
    )
    issue = (
        "Failure"     if any(k in t for k in ["down","not working","failed","failure","unreachable","unavailable"]) else
        "Crash"       if any(k in t for k in ["crash","crashed","stopped","freezing"]) else
        "Performance" if any(k in t for k in ["slow","latency","lag","timeout"]) else
        "Error"       if any(k in t for k in ["error","bug","exception","invalid"]) else
        "Access"      if any(k in t for k in ["unable","cannot","can't","blocked","locked"]) else
        "N/A"
    )
...
def predict_full(ticket: str) -> dict:
    ...
    # Rule-based takes precedence over low-confidence ML
    category    = rule_label if rule_label != "General Support" else model_label

    # When rule fires, boost confidence display (rule was explicit keyword match)
...
def build_ai_text(r: dict) -> str:
    """Plain-text report rendered inside <pre> — no nested HTML."""
...
def render_chat_bubble_ai(r: dict):
    text = build_ai_text(r)
...
def render_chat_bubble_user(text: str):
    import html as _html
    safe = _html.escape(text)
    st.markdown(
...
def render_insights(r: dict):
    ...
    # Confidence bars
    cat_w  = int(r["confidence"])
    rout_w = int(r["routing_conf"])
        ...
        f"""<div class="icard">
              ...
              <div class="conf-wrap">
                ...
                <div class="conf-track">
                  <div class="conf-fill" style="width:{cat_w}%;background:{col_c};"></div>
                </div>
              </div>
              <div class="conf-wrap" style="margin-top:8px;">
                ...
                <div class="conf-track">
                  <div class="conf-fill" style="width:{rout_w}%;background:#1f6feb;"></div>
                </div>
              </div>
            </div>""",
        unsafe_allow_html=True,
    ...
    # Solutions
    items = "".join(
        f'<li><span class="sol-dot">▸</span>{s}</li>'
        for s in r["solutions"]
    )
    st.markdown(
        f"""<div class="icard">
              <div class="icard-label">Past Resolutions</div>
              <ul class="sol-list">{items}</ul>
            </div>""",
        unsafe_allow_html=True,
    )
    ...
    if r["decision"] == "Auto Assigned":
        badge = f'<span class="dec-ok">✔&nbsp; Auto Assigned</span>'
    else:
        badge = f'<span class="dec-warn">⚠&nbsp; Human Review Required</span>'
    st.markdown(
...
# SESSION STATE
# ─────────────────────────────────────────────────────────────
# messages: list of {"role": "user"|"assistant", "text": str, "result": dict|None}
...
# ─────────────────────────────────────────────────────────────
# LAYOUT  — LEFT: insights │ RIGHT: chat
# ─────────────────────────────────────────────────────────────
info_col, chat_col = st.columns([1, 2], gap="medium")

# ── LEFT: Ticket Insights ─────────────────────────────────────
with info_col:
    st.markdown(
...
with chat_col:
    ...
    # Replay conversation from session state
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            render_chat_bubble_user(msg["text"])
        else:
            # Re-render from stored result dict (never store raw HTML)
            render_chat_bubble_ai(msg["result"])
    ...
    if user_input and user_input.strip():
        ...
        render_chat_bubble_user(user_input)
